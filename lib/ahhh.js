...
Ahhh.prototype.update = function(self, opts) {
  ...
  // do hit detections
  self.game.physics.arcade.overlap(self.player, self.obstacles, self.collision,
    null, self);
  self.game.physics.arcade.overlap(self.player, self.ground, self.collision,
    null, self);
...
Ahhh.prototype.reset = function() {
  this.score = -1;
  this.addScore();
  this.obstacles.children.forEach(function(child) {
    child.destroy();
